...
from modules.module_visual import BBXModel, COURTModel, VisualModel, VisualConfig, VisualOnlyMLMHead, FEAModel
...
class NormalizeFEA(nn.Module):
    def __init__(self, config):
        super(NormalizeFEA, self).__init__()
        self.visual_norm2d = LayerNorm(config.vocab_size)
...
class NormalizeBBX(nn.Module):
    def __init__(self, task_config):
        ...
        self.visual_norm2d = LayerNorm(768)
...
class NormalizeCourt(nn.Module):
    def __init__(self, task_config):
        super(NormalizeCourt, self).__init__()
...
class UniVL(UniVLPreTrainedModel):
    def __init__(self, bert_config, visual_config, cross_config, decoder_config, 
                task_config, 
                #  feature_config=['bbx', 'courtseg', 'keypoint', 'allplayer', 'allplayer2']
                feature_config=['bbxcls2', 'bbxball', 'bbxbasket', 'courtseg']
                # feature_config=['bbxcls2']

                ):
        '''
        Is feature config just a list? => How to declare this argument?
        '''
        ...
        features_json = {}
        normalize_feas = {}
        for fea_name in feature_config:
            json_path = os.path.join('./modules', 'visual-base',fea_name+'_config.json')
            feature_config = VisualConfig(json_path)
            features_json[fea_name] = FEAModel(feature_config).cuda()
            normalize_feas[fea_name] = NormalizeFEA(feature_config).cuda()
        self.features = torch.nn.ModuleDict(features_json)
        self.normalize_feas = torch.nn.ModuleDict(normalize_feas)
    ...
    def forward(self, input_ids, token_type_ids, attention_mask, video, video_mask=None,
                ...
                input_caption_ids=None, decoder_mask=None, output_caption_ids=None,task_type=None, feature_tuple=None, feature_tuple_mask=None):
        ...
        for (name1, fea), (name2, mask) in zip(feature_tuple.items(), feature_tuple_mask.items()):
            if name1 == 'allplayer':
                "First, collapse numFrames to batch"
                fea = fea.view(-1, 10, 768) # [batch * numFrame, 10, 768]
                mask = mask.view(-1, 10)
                fea = self.get_fea_output(fea, mask, name1).sum(-2)

                "Then, recover it and run again on seq dimension"
                fea = fea.reshape(-1, 30, 768)
                mask = mask.reshape(-1, 30, 10).sum(-1)
                mask = torch.where(mask !=0, 1, 0)
                fea = self.get_fea_output(fea, mask, name1, allplayer2=True) # use different encoder for this purpose;
                fea_output_tuple[name1] = fea
                fea_output_mask_tuple[name1] = mask
            else:
                fea_output_tuple[name1] = self.get_fea_output(fea, mask, name1, False)
                fea_output_mask_tuple[name1] = mask
        ...
        if self.training:
            ...
            if self._stage_two:
                if self.task_config.do_pretrain:
                    ...
                    cross_output, pooled_output, concat_mask = self._get_cross_output(sequence_output_alm, visual_output_alm, court_output, attention_mask, video_mask, court_mask)
                ...
                if (input_caption_ids is not None) and \
                    ...
                    if self.task_config.do_pretrain:
                        ...
                        decoder_scores, res_tuples = self._get_decoder_score(sequence_output, visual_output,
                                                                             input_ids, attention_mask, video_mask,
                                                                             input_caption_ids, decoder_mask, fea_output_tuple, fea_output_mask_tuple, shaped=True,task_type = task_type)
                    else:
    ...
    def get_fea_output(self, input, input_mask, name, shaped=False, task_type=None, allplayer2=False):
        if allplayer2:
            name = 'allplayer2'
        if shaped is False:
            input = self.normalize_feas[name](input)
        outupt_layers, _ = self.features[name](input.squeeze(), input_mask.squeeze(), output_all_encoded_layers=True,task_type=task_type)
        output = outupt_layers[-1]
        return  output
    ...
    def get_sequence_visual_output(self, input_ids, token_type_ids, attention_mask, video, video_mask, shaped=False, task_type = None):
        ...
        visual_layers, _ = self.visual(video, video_mask, output_all_encoded_layers=True,task_type=task_type)
        visual_output = visual_layers[-1]

        return text_output, visual_output

    def _get_cross_output(self, visual_output, feature_tuples, attention_mask, video_mask, feature_masks):
        list_fea_output = []
        for key, value in feature_tuples.items():
            list_fea_output.append(value)
        "Append visual output to the end"
        list_fea_output.append(visual_output)

        list_fea_mask = []
        for key, value in feature_masks.items():
            list_fea_mask.append(value.squeeze())
        "Append visual mask to the end"
        list_fea_mask.insert(0, video_mask)

        concat_features = torch.cat(list_fea_output, dim=1)  # concatnate tokens and frames
        concat_mask = torch.cat(list_fea_mask, dim=1)
        text_type_ = torch.ones_like(attention_mask)
        concat_type = torch.ones_like(concat_mask)

        "Prepare concat type, start from 0 to numFeature"
        type_list = []
        for idx, _ in enumerate(range(len(list_fea_mask))):
            type_list.append(idx * torch.ones_like(list_fea_mask[0]).int())
        concat_type = torch.cat(type_list, -1)
    ...
    def _get_decoder_score(self, sequence_output, visual_output, input_ids, attention_mask, video_mask, input_caption_ids, decoder_mask, feature_tuple, feature_mask, shaped=False,task_type = None):
        ...
        cross_output, pooled_output, concat_mask = self._get_cross_output(visual_output, feature_tuple, attention_mask, video_mask, feature_mask)
    ...
    def decoder_caption(self, sequence_output, visual_output, input_ids, attention_mask, video_mask, input_caption_ids, decoder_mask, feature_tuple, feature_mask_tuple, shaped=False, get_logits=False,task_type = None):
        if shaped is False:
            input_ids = input_ids.view(-1, input_ids.shape[-1])
            attention_mask = attention_mask.view(-1, attention_mask.shape[-1])
            video_mask = video_mask.view(-1, video_mask.shape[-1])
        ...
        decoder_scores, _ = self._get_decoder_score(sequence_output, visual_output,
                                                    input_ids, attention_mask, video_mask,
                                                    input_caption_ids, decoder_mask, feature_tuple, feature_mask_tuple, shaped=True,task_type = task_type)
