...
class UniVL(UniVLPreTrainedModel):
    ...
    def forward(self, input_ids, token_type_ids, attention_mask, video, video_mask=None,
        ...
        bbx_output = self.get_bbx_output(bbx,bbx_mask)
        # sequence_output is [batch, length, 768]
        # visual_output is [batch, length, 768]
    ...
    def get_bbx_output(self, bbx, bbx_mask, shaped=False, task_type=None):
        if shaped is False:
            bbx = self.normalize_bbx(bbx)
        bbx_layers, _ = self.bbx(bbx, bbx_mask, output_all_encoded_layers=True,task_type=task_type)
