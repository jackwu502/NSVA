...
class OURDS_PT_DataLoader(Dataset):
    """
    ...
    def _get_video(self, idx, s, e, only_sim=False):
        ...
        #print(list(self.csv.keys()))
        feature_file = os.path.join(self.features_path, self.csv["video_id"].values[idx]+'.npy')
        #video_features = np.load(feature_file)
        try:
            video_features = np.transpose(np.load(feature_file))
